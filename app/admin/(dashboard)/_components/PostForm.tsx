...
const CATEGORIES = ['Impermeabilização', 'Pisos', 'Telhados', 'Limpeza', 'Dicas']
...
export default function PostForm({ post }: { post?: BlogPost }) {
  ...
  const [category, setCategory] = useState(post?.category ?? 'Impermeabilização')
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <section className="bg-gray-900 rounded-2xl p-6 space-y-4 border border-gray-800">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Ex: Como impermeabilizar pisos externos em condomínios"
            className="w-full bg-gray-800 text-white rounded-xl px-4 py-3 text-lg font-medium outline-none focus:ring-2 focus:ring-blue-600 placeholder-gray-600"
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="como-impermeabilizar-pisos"
              className="w-full bg-gray-800 text-gray-300 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-blue-600 font-mono"
            />
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-gray-800 text-white rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-blue-600"
            >
              {CATEGORIES.map((c) => (
          ...
          <label
            className={`flex flex-col items-center justify-center border-2 border-dashed rounded-xl py-10 cursor-pointer transition-colors ${
              uploading
                ...
                : 'border-gray-700 hover:border-blue-600/50 hover:bg-gray-800/50'
            }`}
          >
            <input
              type="file"
            ...
            />
            {uploading ? (
              <p className="text-blue-400 text-sm font-medium">Enviando...</p>
            ) : (
              <>
      ...
      <section className="bg-gray-900 rounded-2xl p-6 space-y-4 border border-gray-800">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Ex: impermeabilização de pisos Balneário Camboriú"
            className="w-full bg-gray-800 text-white rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-blue-600"
          />
        </div>
        ...
        <div>
          <div className="flex justify-between mb-1.5">
            ...
            <span
              className={`text-xs ${metaTitle.length > 60 ? 'text-red-400' : 'text-gray-500'}`}
            >
              {metaTitle.length}/60
            </span>
          </div>
        ...
        <div>
          <div className="flex justify-between mb-1.5">
            ...
            <span
              className={`text-xs ${
                metaDescription.length > 160 ? 'text-red-400' : 'text-gray-500'
              }`}
            >
          ...
          <textarea
            ...
            onChange={(e) => {
              ...
              setMetaDescManual(true)
            }}
            placeholder="Descrição que aparece no Google (máx. 160 caracteres)"
            rows={3}
            className="w-full bg-gray-800 text-white rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-blue-600 resize-none"
          />
        </div>
      </section>
      ...
      <section className="bg-gray-900 rounded-2xl p-6 space-y-4 border border-gray-800">
        ...
        {sections.map((section, i) => (
          <div key={i} className="bg-gray-800 rounded-xl p-4 space-y-3 border border-gray-700">
            <div className="flex items-center gap-2">
              <span className="text-blue-400 text-xs font-bold bg-blue-600/10 px-2 py-0.5 rounded">
                H2 — Seção {i + 1}
              </span>
              {sections.length > 1 && (
                <button
            ...
            <input
              ...
              className="w-full bg-gray-700 text-white rounded-lg px-4 py-2.5 text-sm font-medium outline-none focus:ring-2 focus:ring-blue-600"
            />
            <textarea
              value={section.body}
              ...
              rows={8}
              className="w-full bg-gray-700 text-gray-300 rounded-lg px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-600 resize-y leading-relaxed"
            />
          </div>
        ))}
        ...
        <button
          type="button"
          onClick={addSection}
          className="w-full border-2 border-dashed border-gray-700 hover:border-blue-600/50 text-gray-500 hover:text-blue-400 rounded-xl py-4 text-sm font-medium transition-colors"
        >
          + Adicionar Seção
        </button>
      </section>
      ...
      <section className="bg-gray-900 rounded-2xl p-6 border border-gray-800">
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={loading}
            className="flex-1 bg-blue-600 hover:bg-blue-500 text-white font-bold py-4 rounded-xl transition-colors disabled:opacity-50 text-sm"
          >
